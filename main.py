...
def Generating_binary_words(block):
    words = []
    for i in range(8):
...
def Forming_of_Converted_Blocks(b):
    ...
    ready_blocks = []
    for block in blocks:
        curr_block = [0 for _ in range(32)]
        for l in range(32):
            for k in range(8):
...
str = '703aace7'
str_bytes = [str[2 * i:2 * i + 2] for i in range(4)]
str_bytes = [int(x, 16) for x in str_bytes]
str_byte = Trans(str_bytes)
