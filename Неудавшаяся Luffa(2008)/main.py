...
func_inj5 = [[15, 8, 10, 10, 8, 1], [8, 15, 8, 10, 10, 2], [10, 8, 15, 8, 10, 4], [10, 10, 8, 15, 8, 8],
             [8, 10, 10, 8, 15, 16]]

func_inj = [func_inj3, func_inj4, func_inj5] # список, задающий коэффициенты для функции MI

# Блоки для замены SubCrumb
blocks_for_subcrumb = ['0111', '1101', '1011', '1010', '1100', '0100', '1000', '0011', '0101', '1111', '0110', '0000',
                       '1001', '0001', '0010', '1110']

v0 = [109, 37, 30, 105, 68, 176, 81, 224, 78, 170, 111, 180, 219, 247, 132, 101, 110, 41, 32, 17, 144, 21, 45, 244, 238,
      5, 129, 57, 222, 246, 16, 187] # 0 в спецификации
v1 = [195, 180, 75, 149, 217, 210, 242, 86, 112, 238, 233, 160, 222, 9, 159, 163, 93, 155, 5, 87, 143, 201, 68, 179,
      207, 28, 207, 14, 116, 108, 213, 129] # 1 в спецификации
v2 = [247, 239, 200, 157, 93, 186, 87, 129, 4, 1, 108, 229, 173, 101, 156, 5, 3, 6, 25, 79, 102, 109, 24, 54, 36, 170,
      35, 10, 139, 38, 74, 231] # 2 в спецификации
v3 = [133, 128, 117, 213, 54, 215, 156, 206, 229, 113, 247, 215, 32, 75, 31, 103, 53, 135, 12, 106, 87, 233, 233, 35,
      20, 188, 184, 8, 124, 222, 114, 206]
v4 = [108, 104, 233, 190, 94, 196, 30, 34, 200, 37, 183, 199, 175, 251, 67, 99, 245, 223, 57, 153, 15, 198, 136, 241,
      176, 114, 36, 204, 3, 232, 108, 234]

initial_vectors = [v0, v1, v2, v3, v4] # список состоящий из констант для первого раунда
...
# Также чередуются
consts_0 = [809079974, 3761469464, 3236319897, 1142663437, 1824733714, 2134168642, 3696662590, 2475237759, 503320719,
            3853040870, 2013282877, 1383381748, 2405136514, 646486951, 2531384082, 2585947805]

consts_1 = [3068006637, 23617341, 1895070382, 94469364, 117941204, 3171535562, 471764817, 4096207656, 1887059269,
            340452812, 2930935138, 4205293099, 3133805961, 776532417, 1084518206, 3106129668]

consts_2 = [4230011346, 3797840577, 877997605, 3861101426, 2061009295, 1549313188, 2218292810, 507044583, 3144540210,
            2028178333, 3988226248, 660104985, 3649336150, 921544063, 2730984500, 1882893543]

consts_3 = [2987634597, 3760769471, 3360603797, 1148546961, 1314949666, 2123353650, 1457019134, 2506442942, 876286863,
            4263058402, 3505147453, 1018308325, 753617026, 1497670286, 3014468104, 2714026837]

consts_4 = [4040354275, 1351669111, 2886850554, 756622763, 466314994, 3026491052, 1865259977, 3516029616, 2019567177,
            689117878, 2396711250, 264262595, 996910408, 1057050380, 3987641632, 4228201521]

consts = [consts_0, consts_1, consts_2, consts_3, consts_4] # константы для функции AddConstant
...
def Generating_binary_words(block): # переводим блок в двоичное битовое представление (по словам)
    words = []
    for i in range(8):
        curr_block = block[4 * i:4 * i + 4]
        curr_block = Trans(curr_block)
        words.append(curr_block)
    return words

def Forming_of_Converted_Blocks(b):
    blocks = [Generating_binary_words(x) for x in b]
    ready_blocks = []
    for block in blocks: ## взяли блок
        curr_block = [0 for _ in range(32)] ## текущий сформированный пустой
        for l in range(31, -1, -1): ## от 0 до 31
            for k in range(8): ## от 0 до 7
                curr_block[31-l] += int(block[k][l]) * (2**k)
        ready_blocks.append(curr_block)
    return ready_blocks
...
def Multi_Matrix(b, w):  # b - blocks
    b = Forming_of_Converted_Blocks(b)
    injection = func_inj[0]  # выбираем нужные векторы !!!!!!!!!!!!!!!!!!!!!!!!!!!
...
def XOR_of_blocks(a, b):
    return [Add(a[i], b[i]) for i in range(32)]

def Perform(x):
    curr_byte = bin(x)[2:]
    curr_byte = curr_byte[::-1]
    byte = [int(x) for x in list(curr_byte)]
    while len(byte) != 8:
        byte.append(0)
    return byte
...
def MI_func(blocks):
    xor_or_all, ready_blocks = [0 for _ in range(32)], []
    xor_or_all = XOR_of_blocks(xor_or_all, blocks[0])
    xor_or_all = XOR_of_blocks(xor_or_all, blocks[1])
    xor_or_all = XOR_of_blocks(xor_or_all, blocks[2])
    first = [opp_Perform(Multiplic_2(Perform(x))) for x in xor_or_all]
    for j in range(3):
        third = blocks[-1].copy()
        for k in range(j):
            third = [opp_Perform(Multiplic_2(Perform(x))) for x in third]
        ready_block = XOR_of_blocks(third, first)
        ready_block = XOR_of_blocks(ready_block, blocks[j])
...
def Add_Constant(a0, a4, num_of_block, num_of_round):
    a0, a4 = int(Trans(a0), 2), int(Trans(a4), 2)  ## перевод в число из массива четырёх байт
    add1, add2 = consts[num_of_block][2 * num_of_round], consts[num_of_block][2 * num_of_round + 1]  ## выбор соотв. константы
    a0, a4 = Add(a0, add1), Add(a4, add2)
    return opp_Trans(bin32(a0)), opp_Trans(bin32(a4))

def Permutation(b):  ## на входе блоки 1..w
    ready_blocks = []
    for j, curr_block in enumerate(b):  ## j изменяется от 0 до 2
        words = []
        for i in range(8):
...
def Tweak(words): ## реализовать побыстрее
    for j in range(4, 8):
